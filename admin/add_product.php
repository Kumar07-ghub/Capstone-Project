...
// If no upload but image text is given
elseif (!empty($_POST['image_text'])) {
    $imagePath = trim($_POST['image_text']);
}

// If neither provided
else {
    $error = 'Please upload an image or enter image text/URL.';
}
    if (!$error) {
...
<body>
    ...
    <div class="mb-3">
    <label for="image" class="form-label fw-semibold">Product Image (Upload)</label>
    <input type="file" id="image" name="image" class="form-control" accept="image/png, image/jpeg, image/gif">
    <div class="form-text">Optional: You can either upload an image or enter a text/URL below.</div>
    <img id="imagePreview" src="#" alt="Image Preview" class="img-preview d-none" />
    </div>

<div class="mb-3">
    <label for="image_text" class="form-label fw-semibold">Or Enter Image URL / Label</label>
    <input type="text" id="image_text" name="image_text" class="form-control"
        value="<?= isset($_POST['image_text']) ? htmlspecialchars($_POST['image_text']) : '' ?>"
        placeholder="e.g. https://example.com/img.jpg or sample-image.jpg or 'No image available'">
</div>
